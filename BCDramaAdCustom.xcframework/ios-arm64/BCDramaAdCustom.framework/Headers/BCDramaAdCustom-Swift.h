...
#ifndef BCDRAMAADCUSTOM_SWIFT_H
#define BCDRAMAADCUSTOM_SWIFT_H
...
# undef any
# pragma clang attribute push(__attribute__((external_source_symbol(language="Swift", defined_in="BCDramaAdCustom",generated_declaration))), apply_to=any(function,enum,objc_interface,objc_category,objc_protocol))
...
/// 自定义广告宿主协议。
/// 宿主实现本协议后，将实例赋给 <code>BCCustomAdAdapter.delegate</code>，由 Adapter 在 SDK 请求广告时转发调用。
/// 广告加载成功、关闭或失败时，建议调用 <code>BCVideoManager.customAdStatusChanged</code> 同步状态给 SDK。
SWIFT_PROTOCOL("_TtP15BCDramaAdCustom18BCCustomAdDelegate_")
@protocol BCCustomAdDelegate <NSObject>
/// 注册宿主侧广告 SDK。
/// SDK 初始化流程中会调用，用于初始化宿主接入的第三方广告平台（如优量汇、穿山甲等）。
/// \param appId BC SDK 的 appId，可用于关联宿主广告平台配置
///
- (void)bc_customAdRegisterSDKWithAppId:(NSString * _Nonnull)appId;
/// 加载 Banner 横幅广告。
/// \param viewController 用于展示与点击跳转的上下文控制器
///
/// \param container Banner 广告容器视图，宿主应将广告视图添加到此容器中
///
/// \param placementId 广告位 ID
///
/// \param pageType 广告所在页面类型（如播放页、推荐页等）
///
/// \param onClose 广告关闭或加载失败时的回调
///
/// \param onShow 广告展示成功时的回调，参数通常为 eCPM 或展示状态标识
///
...
/// \param viewController 用于展示与点击跳转的上下文控制器
///
/// \param container 信息流广告 Cell，宿主应将渲染后的广告视图放入 <code>container.contentView</code>
///
/// \param pageType 广告所在页面类型
///
/// \param placementId 广告位 ID
///
/// \param completion 加载完成回调；成功时返回广告视图与 placementId，失败时 <code>error</code> 非空
///
...
/// 加载并展示插屏（全屏）广告。
/// \param viewController 用于展示插屏广告的上下文控制器
///
/// \param placementId 广告位 ID
///
/// \param pageType 广告所在页面类型
///
/// \param onShow 广告展示成功时的回调
///
/// \param onClose 广告关闭时的回调
///
/// \param onFailed 广告加载或展示失败时的回调
///
...
/// 加载激励视频广告。
/// 由 <code>BCCustomRewardVideoViewController</code> 在展示占位页后触发，宿主负责拉取并播放激励视频。
/// \param viewController 激励视频占位页控制器，广告应在此控制器上展示
///
/// \param placementId 广告位 ID
///
/// \param taskId 激励任务 ID，用于关联解锁等业务逻辑
///
/// \param onClose 激励视频关闭时的回调（用于隐藏 SDK 侧 loading 等 UI）
///
/// \param onShow 激励视频开始展示时的回调，参数通常为 eCPM 或展示状态标识；
/// 激励是否有效请通过 <code>BCVideoManager.customAdStatusChanged</code> 的 <code>isEffective</code> 上报
///
